...
Build each general-purpose explorer agent's prompt from this template. Fill in the placeholders.
...
Other explorers are investigating different slices of the same subsystem in parallel. Stay within your assigned angle and go deep.
...
Start by finding the relevant code. Use `find` to locate files, `ls` to inspect directories, `grep` to find symbols, and `read` to inspect implementations. Verify behavior from the code rather than inferring it from names.
